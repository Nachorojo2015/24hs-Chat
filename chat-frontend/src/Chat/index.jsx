    ...
    const fetchData = async () => {
      try {
        ...
        if (login.isConfirmed) {
              ...
              if (u["username"] !== login.value.login) {
                if (u["message"][1] === "a") {
                  ...
                  <div class="flex items-center gap- mt-2">
                    <img src=${
                      u["user-image"]
                    } alt="" class="align-middle w-[50px] h-[50px] border inline rounded-[50%]"/>
                    <audio src=${
                      u["message"][0]
                    } controls class="mt-2 ml-2"></audio>
                  </div>
                  <p class="ml-16 mt-2">${
                    days[new Date().getDay()] !== u["time"].split(",")[1].trim()
                      ? "last "
                      : ""
                  }${u["time"]}</p>
                  `;
                  ...
                  containerMessages.innerHTML += `
                      <div class="flex items-center gap-3 mt-2">
                        ...
                        <img src=${
                          u["message"][0]
                        } alt="capture-picture-user" class="w-[70%] h-[70%] md:w-[30%] md:h-[30%]" />
                      </div>
                      <p class="ml-16 mt-2">${
                  ...
                  containerMessages.innerHTML += `
                      <div class="flex items-center gap-3">
                        ...
                        <p class="bg-orange-500 rounded-3xl p-2">${
                          u["message"][0]
                        }</p>
                      </div>
                      <p class="ml-16 mt-1">${
                  ...
                  containerMessages.innerHTML += `
                      <div class="flex flex-wrap items-center gap-3 mt-2">
                        ...
                        <video src=${
                          u["message"][0]
                        } width="640" height="480" controls autoplay></video>
                      </div>
                      <p class="ml-16 mt-2">${
                        days[new Date().getDay()] !==
                        u["time"].split(",")[1].trim()
                  ...
                  const fileExtension = u["message"][0]
                    .split("/")
                    .slice(-1)[0]
                    .split(".")[1];
                  if (
                    "webm mp4 avi mkv flv mov divx h.264 xvid rm".includes(
                      fileExtension
                    )
                  ) {
                    const days = [
                ...
                if (u["message"][1] === "a") {
                  ...
                  <a href=${
                    u["message"][0]
                  } target="_BLANK" class="ml-auto bg-red-500 p-2 rounded-md"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-script inline mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M17 20h-11a3 3 0 0 1 0 -6h11a3 3 0 0 0 0 6h1a3 3 0 0 0 3 -3v-11a2 2 0 0 0 -2 -2h-10a2 2 0 0 0 -2 2v8" /></svg>${
                    u["message"][0].split("/").slice(-1)[0]
                  }</a>
                  </div>
                  `;
          ...
          const buttonSendMessage = document.getElementById(
            "send-message-button"
          );
          ...
          const sendVoiceMessage =
            document.getElementById("stop-voice-message");
          let mediaRecorder;
          let audioChunks = [];
          navigator.mediaDevices
            ...
            .then((stream) => {
              ...
              mediaRecorder.onstop = () => {
                ...
                const audioFile = new File([audioBlob], `${randomNumber}.mp3`, {
                  type: "audio/mp3",
                });
                  ...
                  try {
                    ...
                    if (response.ok) {
                      ...
                      socket.emit("new-message", {
                        ...
                        "user-image": login.value.avatar_url,
                        message: [
                          `http://localhost:3000/uploads/audio/${f.name}`,
                          "a",
                        ],
                        time: formatTime(),
                      });
          ...
          cameraButton.addEventListener("click", () => {
            ...
            const capturePictureButton =
              document.getElementById("capture-picture");
            ...
            const cancelPictureButton =
              document.getElementById("cancel-picture");
            ...
            function sendVideo() {
              ...
              fetch(videoUrl)
                ...
                .then((blob) => {
                  ...
                  const videoFile = new File([blob], `${randomNumber}.mp4`, {
                    type: "video/mp4",
                  });
                  ...
                  files.forEach(async (f) => {
                    ...
                    try {
                      const response = await fetch(
                        "http://localhost:3000/images-videos",
                        {
                          method: "POST",
                          body: formData,
                        }
                      ...
                      if (response.ok) {
                        ...
                        socket.emit("new-message", {
                          ...
                          "user-image": login.value.avatar_url,
                          message: [
                            `http://localhost:3000/uploads/media/${f.name}`,
                            "med",
                          ],
                          time: formatTime(),
                        });
          ...
          const sendFilesDocumentButton = document.getElementById(
            "send-files-document"
          );
          const sendImageVideosButton =
            document.getElementById("send-images-videos");
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  socket.emit("new-message", {
                    ...
                    "user-image": login.value.avatar_url,
                    message: [
                      `http://localhost:3000/uploads/documents/${f.name}`,
                      "doc",
                    ],
                    time: formatTime(),
                  });
  ...
  return (
    <>
      <section className="relative">
        <nav className="flex items-center justify-between bg-[rgb(35,34,34)] p-3">
          <div className="flex gap-5 items-center">
            <img
              src={user["user-image"]}
              alt="user-image"
              className="align-middle w-[50px] h-[50px] rounded-[50%]"
            />
            <p>{user["username"]?.toUpperCase()}</p>
          </div>
          <div>
            <p className="text-sm">Users Conected: {quantityUsers}</p>
          </div>
          <div className="items-center gap-5 hidden" id="camera-tools">
            <button id="capture-picture">
              <svg
        ...
        <div
          className={
            isPickerVisible ? "block absolute bottom-3 left-3" : "hidden"
          }
        >
          <EmojiPicker
            data={data}
        ...
        <div
          className="absolute right-2 md:right-[1300px] bottom-1 bg-slate-900 p-1 rounded-md hidden"
          id="files"
        >
          <ul>
            <li className="p-2" onClick={handleFileClick}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="icon icon-tabler icon-tabler-file-export inline mr-2 cursor-pointer"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                strokeWidth="2"
            ...
            <button
              className="ml-3 bg-orange-500 p-[2px] rounded-sm hidden"
              id="send-files-document"
            >
              Enviar
            </button>
            <li className="p-2" onClick={handleImageClick}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="icon icon-tabler icon-tabler-photo-scan inline mr-2 cursor-pointer"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                strokeWidth="2"
            ...
            <button
              className="ml-3 bg-orange-500 p-[2px] rounded-sm hidden"
              id="send-images-videos"
            >
              Enviar
            </button>
            <li className="p-2" onClick={handleAudioClick}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="icon icon-tabler icon-tabler-device-speaker inline mr-2 cursor-pointer"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
            ...
            <li className="p-2" id="photo">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="icon icon-tabler icon-tabler-camera inline mr-2 cursor-pointer"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                strokeWidth="2"
        ...
        <video
          ...
          height="480"
          autoPlay
          className="absolute top-60 md:top-24 md:right-[30%]"
          id="camera"
        ></video>
        <canvas
          id="photoCanvas"
        ...
        ></canvas>
        <div
          className="absolute top-60 md:top-24 md:right-[30%] hidden"
          id="picture-preview"
        >
        ...
        <footer className="flex justify-around fixed w-full bottom-2 items-center md:p-2 mt-9 md:mt-0">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="icon icon-tabler icon-tabler-mood-happy-filled md:relative absolute left-3 cursor-pointer"
            onClick={emoji}
            width="24"
            height="24"
            viewBox="0 0 24 24"
          ...
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="icon icon-tabler icon-tabler-plus md:relative absolute right-12 md:right-0 cursor-pointer"
            onClick={openFilesSection}
            width="24"
            height="24"
            viewBox="0 0 24 24"
            strokeWidth="2"
          ...
          <input
            ...
            id="send-message"
            className="md:w-[80%] w-full p-3 rounded-3xl border-none text-white md:indent-0 indent-10"
          />
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="icon icon-tabler icon-tabler-send-2 md:relative absolute right-3 cursor-pointer hidden"
          ...
          <button
            ...
            className="md:relative absolute right-3 cursor-pointer"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            id="stop-voice-message"
            className="md:relative absolute right-3 cursor-pointer hidden"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
